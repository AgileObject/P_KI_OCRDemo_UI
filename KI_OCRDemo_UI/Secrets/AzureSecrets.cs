...
namespace KI_OCRDemo_UI.Azure_Secrets
{
    class AzureSecrets
    {
        #region Declaration
        ...
        public static bool LoadSucceeded(string[] args)
        {
            ...
            if (loadSucceeded)
            {
                ...
                _Logger.Info(" Using computer vision key: " + maskedKey);
                _Logger.Info(" Using computer vision endpoint: " + Endpoint);
            }
        ...
        private static bool LoadKeySucceeded(string[] args)
        {
            ...
            if (Key == null)
            {
                try
                {
                    Key = ConfigurationManager.AppSettings["AzureVisionKey"].ToString();
                }
                catch
                {
                    // Ignore
        ...
        private static bool LoadEndpointSucceeded(string[] args)
        {
            ...
            if (Endpoint == null)
            {
                try
                {
                    Endpoint = ConfigurationManager.AppSettings["AzureVsionEndpoint"].ToString();
                }
                catch
                {
                    // Ignore
